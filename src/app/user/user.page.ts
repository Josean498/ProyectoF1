...
export class UserPage implements OnInit {
  ...
  clicBotonInsertar() {
    this.firestoreService.insertar("pilotos", this.document.data).then(() => {
      console.log('Piloto creada correctamente!');
      this.pilotosEditando = {} as Pilotos;
    }, (error) => {
      console.error(error);
    });
...
async presentAlertConfirmInsertar() {
		const alert = await this.alertController.create({
			header: 'Confirmar',
			message: '¿Quieres añadir la pieza <strong>'+ this.document.data.nombre +'</strong>?',
			buttons: [
				{
					text: 'Descartar',
					cssClass: 'secondary',
...
async presentAlertConfirmModificar() {
		const alert = await this.alertController.create({
			header: 'Confirmar',
			message: '¿Quieres confirmar los cambios en la pieza <strong>'+ this.document.data.nombre +'</strong>?',
			buttons: [
				{
					text: 'Descartar',
					cssClass: 'secondary',
...
async presentAlertConfirmBorrar() {
		const alert = await this.alertController.create({
			header: 'Confirmar',
			message: '¿Quieres borrar la pieza <strong>'+ this.document.data.nombre +'</strong>?',
			buttons: [
				{
					text: 'Descartar',
					cssClass: 'secondary',
